    ...
    if (!m_is_advertising)
    {
        ...
        if (m_ble_peer_data_valid)
        {
            ...
            if (memcmp(m_ble_peer_data.irk.irk, empty_irk.irk, sizeof(empty_irk.irk)) == 0)
            {
                ...
                m_adv_params.p_peer_addr = &m_ble_peer_data.peer_addr;
            ...
            else
            {
                ...
                p_addr[0] = &m_ble_peer_data.peer_addr;
        ...
        case BLE_GAP_EVT_SEC_PARAMS_REQUEST:
            {
                ...
                id_key.id_addr_info = m_ble_peer_data.peer_addr;
...
uint32_t dfu_transport_update_start(void)
{
    ...
    if (err_code == NRF_SUCCESS)
    {
        err_code = sd_ble_gap_address_set(BLE_GAP_ADDR_CYCLE_MODE_NONE, &(m_ble_peer_data.device_addr));
    ...
    else
    {
        ...
        // Increase the second byte in the BLE address when advertising openly.
        addr.addr[1] += 1;
