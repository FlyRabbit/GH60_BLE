...
static dfu_ble_peer_data_t m_peer_data __attribute__((at(0x20003F80))) ;        /**< This variable should be placed in a non initialized RAM section in order to be valid upon soft reset from application into bootloader. */
static uint16_t            m_peer_data_crc __attribute__((at(0x20003F80+sizeof(dfu_ble_peer_data_t)))) ;        /**< CRC variable to ensure the integrity of the peer data provided. */
